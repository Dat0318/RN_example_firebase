...
export type StorageGetFunc<T> = (
  key: string,
  option?: AppModule.MMKVOption,
) => Promise<T | null>;

export type StorageSetFunc<T> = (
  key: string,
  value: string,
  option?: AppModule.MMKVOption,
) => Promise<T | null>;
 ...
 * @param key The key to fetch.
 */
export async function loadNumber(key: string, option?: AppModule.MMKVOption) {
  try {
    return await AppModule.MMKVStorage.getNumber(
      key,
      StyleSheet.flatten([optionMMKV, option]),
    );
  } catch {
...
export async function saveNumber(
  ...
  try {
    await AppModule.MMKVStorage.setNumber(
      key,
      value,
      StyleSheet.flatten([optionMMKV, option]),
    );
